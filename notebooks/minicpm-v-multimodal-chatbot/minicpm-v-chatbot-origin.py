"""
MiniCPM-V Multimodal Chatbot with OpenVINO（与 notebook 操作逻辑完全一致版）
...
用法:
    ...
    python minicpm-v-chatbot-origin.py

    # 指定其他本地模型路径
    python minicpm-v-chatbot-origin.py --model /path/to/your/model

    # 指定 HuggingFace 在线模型
    python minicpm-v-chatbot-origin.py --model openbmb/MiniCPM-V-4_5

    # 指定推理设备
    python minicpm-v-chatbot-origin.py --device CPU

    # 仅做单次推理（不启动 Gradio）
    python minicpm-v-chatbot-origin.py --image cat.png --question "What is unusual on this image?"

    # 跳过模型转换（OpenVINO 模型目录已存在时使用）
    python minicpm-v-chatbot-origin.py --skip-convert
"""
...
# ── 模型配置 ────────────────────────────────────────────────────────────────────
# 客户提供的本地模型路径（优先使用）
LOCAL_MODEL_PATH = "/home/nvme-data/AI-models/MiniCPM-V-4_5"
...
def get_ov_model_dir(model_id: str) -> Path:
    ...
    本地路径时，在原始模型的父目录下创建 <model_name>-ov 文件夹；
    HuggingFace ID 时，在当前工作目录下创建。
    """
    p = Path(model_id)
    if p.is_absolute() or p.exists():
        return p.parent / (p.name + "-ov")
    return Path(get_model_name(model_id) + "-ov")
...
def convert_model(model_id: str, model_dir: Path):
    ...
    #       "ratio": "0.8",
    #   }
    # 直接调用 Python API 而非 optimum-cli 子进程，避免多版本 optimum 的 PATH 冲突
